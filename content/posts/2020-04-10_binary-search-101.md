...
![https://miro.medium.com/max/600/1*EYkSkQaoduFBhpCVx7nyEA.gif](https://miro.medium.com/max/600/1*EYkSkQaoduFBhpCVx7nyEA.gif)
...
1. `lo` & `hi` We define two variables, let's call them `lo` and `hi` . They will store array indexes and they work like a boundary such that we will only be looking at elements inside the boundary. Normally, we would want initialize the boundary to be the entire array.
   ...
   let lo = 0, hi = nums.length-1;
   ```
2. `mid` The `mid` variable indicates the middle element within the boundary. It separates our boundary into 2 parts. Remember how I said binary search works by keep cutting the elements in half, the `mid` element works like a traffic police, it indicates us which side do we want to cut our boundary.
...
1. Choice of `lo` and `hi`, aka the boundary Normally, we set the initial boundary to the number of elements in the array
   ...
   But this is not always the case. We need to remember: the boundary is the range of elements we will be searching from.
...
3. How do we shrink boundary
   ...
   But what kind of logic are we using here? My rule of thumb is always use a logic that you can **exclude** `mid`. Let's see an example:
